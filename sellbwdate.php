...
$sql = "select prm.productName, sum(quantity), prm.unit from tbl_product_master as prm left join (tbl_sell as tp inner join tbl_sell_master as pm on tp.sellMasterId = pm.id) on prm.id = tp.productId where pm.sellInvoiceDate between $1 and $2 group by(prm.id)";
